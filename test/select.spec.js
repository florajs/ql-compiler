...
describe('select compiler', () => {
    ...
    describe('single deep nested attribute', () => {
        ...
        it('with scalar input', () => {
            const fn = compile.select(select);
            expect(fn({ instrument: { underlying: { id: 42 } } })).to
                .eql({ instrument: { underlying: { id: 42 } } });
        ...
        it('with empty input', () => {
            ...
            expect(fn({})).to.eql({ });
            expect(fn({ instrument: {} })).to.eql({ instrument: {} });
            expect(fn({ instrument: { underlying: {} } })).to
                .eql({ instrument: { underlying: {} } });
        });
    });
    ...
    describe('nested attributes (dot notation)', () => {
        ...
        it('with multiple values present', () => {
            expect(fn({ quote: { bid: { value: 42 }, ask: { value: 23 } } })).to
                .eql({ quote: { bid: { value: 42 } } });
        });
    });
    ...
    describe('nested attributes (mixed notation)', () => {
        ...
        it('with single value present', () => {
            expect(fn({ instrument: { quote: { bid: { value: 42 } } } })).to
                .eql({ instrument: { quote: { bid: { value: 42 } } } });
            expect(fn({ instrument: { quote: { bid: { value: 42, name: 'xy' } } } })).to.eql({ instrument: { quote: { bid: { value: 42 } } } });
            expect(fn({ instrument: { quote: { bid: { value: 42 }, ask: { value: 23 } } } })).to
                .eql({ instrument: { quote: { bid: { value: 42 } } } });
    ...
    describe('multiplied attributes', () => {
        ...
        it('with multiple values present', () => {
            expect(fn({ quote: { bid: { value: 42 }, ask: { value: 23 } } })).to
                .eql({ quote: { bid: { value: 42 }, ask: { value: 23 } } });
        });
    });
    ...
    describe('nested multiplied attributes', () => {
        ...
        it('with single value present', () => {
            ...
            expect(fn({ instrument: { quote: { bid: { value: 42 }, ask: { value: 23 } } } })).to
                .eql({ instrument: { quote: { bid: { value: 42 }, ask: { value: 23 } } } });
